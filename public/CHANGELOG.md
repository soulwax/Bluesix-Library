...
- Organization rail footer shortcuts for both GitHub and changelog pages, placed above the settings action for faster project navigation

### Changed

- Resource card drag now starts from the card surface (while excluding interactive controls), making item moves between categories more consistent
- Card and link drag sources now use a shared drag payload helper so category and sidebar drop targets receive uniform move metadata
- Build pipeline now includes a `prebuild` copy step that mirrors `CHANGELOG.md` into `public/CHANGELOG.md`

### Fixed

- Drag-hold interactions on link rows no longer open links unexpectedly in a new tab when the user intends to drag
