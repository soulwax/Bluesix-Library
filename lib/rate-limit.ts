...
async function executeRedisCommands(
  ...
  const payload = commands.map((command) => createCommandPayload(command)).join("")
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.once("end", () => {
      if (!settled && replies.length < expectedResponses) {
        fail(new Error("Redis connection closed before full response was read"))
      }
    })
...
async function consumeRedisRateLimit(
  ...
  const runWithTlsMode = async (useTls: boolean) => {
    const replies = await executeRedisCommands(config, commands, useTls)
  ...
  try {
    return await runWithTlsMode(config.useTls)
  } catch (primaryError) {
    ...
    const fallbackResult = await runWithTlsMode(true)
...
export async function assertRequestRateLimit(
  ...
  const subject = suffix ? `${baseSubject}:${normalizeSubject(suffix)}` : baseSubject
